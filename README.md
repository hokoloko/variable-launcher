...
*Примечание: Я знаю что это трудно назвать документацией. Буду приводить эту хуйню в порядок до тех пор, пока сам не буду доволен. (лень чето, как нить потом (наверное, я бы победил в конкурсе на звание самого ленивого человека в мире))*
...
- `onAttack(int playerID);` - вызывается когда ты бьешь игрока. `playerID` - айди игрока которого ты ударил. Для этого хука можно использовать `preventDefault();`.
